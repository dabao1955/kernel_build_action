...
# shellcheck disable=SC2086,SC2154,SC2155,SC2046,SC2001,SC2063,SC2002,SC2003,SC2169
...
if [ ${{ inputs.disable-lto }} = true ]; then
    if grep -q "LTO" "arch/$ARCH/configs/$CONFIG"
    then
...
if [ "$AOSP_CLANG" = true ]; then
    ...
    if [ "$CCACHE" = true ]; then
        export USE_CCACHE=1
        make -j$(nproc --all) CROSS_COMPILE="$HOME"/gcc-64/bin/aarch64-linux-android- COMPILE_ARM32="$HOME"/gcc-32/bin/arm-linux-androideabi- CLANG_TRIPLE=aarch64-linux-gnu- CC="ccache clang" ARCH="$ARCH" O=out ${{ inputs.extra-cmd }}
    else
        make -j$(nproc --all) CROSS_COMPILE="$HOME"/gcc-64/bin/aarch64-linux-android- COMPILE_ARM32="$HOME"/gcc-32/bin/arm-linux-androideabi- CLANG_TRIPLE=aarch64-linux-gnu- CC=clang ARCH="$ARCH" O=out ${{ inputs.extra-cmd }}
    fi
    echo "::endgroup::"
    ...
    export PATH="$CLANG_PATH"/bin:"$PATH"
    make -j$(nproc --all) CROSS_COMPILE="$CLANG_PATH"/bin/llvm- COMPILE_ARM32="$CLANG_PATH"/bin/llvm- CLANG_TRIPLE=aarch64-linux-android$(( ${{ inputs.android-version }} + 19 ))- CC=clang ARCH="$ARCH" O=out "$CONFIG"
    if [ "$CCACHE" = true ]; then
        export USE_CCACHE=1
        make -j$(nproc --all) CROSS_COMPILE="$CLANG_PATH"/bin/llvm- COMPILE_ARM32="$CLANG_PATH"/bin/llvm- CLANG_TRIPLE=aarch64-linux-android$(( ${{ inputs.android-version }} + 19 ))- CC="ccache clang" ARCH="$ARCH" O=out ${{ inputs.extra-cmd }}
    else
        make -j$(nproc --all) CROSS_COMPILE="$CLANG_PATH"/bin/llvm- COMPILE_ARM32="$CLANG_PATH"/bin/llvm- CLANG_TRIPLE=aarch64-linux-android$(( ${{ inputs.android-version }} + 19 ))- CC=clang ARCH="$ARCH" O=out ${{ inputs.extra-cmd }}
    fi
    echo "::endgroup::"
    ...
    make -j$(nproc --all) CROSS_COMPILE="$HOME"/gcc-64/bin/aarch64-linux-android- COMPILE_ARM32="$HOME"/gcc-32/bin/arm-linux-androideabi- ARCH="$ARCH" O=out ${{ inputs.extra-cmd }} 
    echo "::endgroup::"
    echo "::endgroup::"
fi
